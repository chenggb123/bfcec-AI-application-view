...
export function Nav() {
  ...
  useEffect(() => {
    ...
    const onDocClick = (e: MouseEvent) => {
      ...
      if (target.closest('[data-nav-links]') || target.closest('[data-hamburger]')) return
      setMenuOpen(false)
    }
  ...
  return (
    <nav className={cn(styles.nav, scrolled && styles.scrolled)}>
      ...
      <Link href="/" className={styles.brand}>
        ...
        <span>{t('brand')}</span>
        <mark className={styles.brandMark}>{t('brandTag')}</mark>
      </Link>

      {/* Nav links + lang toggle */}
      <ul
        className={cn(styles.navLinks, menuOpen && styles.navLinksOpen)}
        data-nav-links
      >
        ...
        <li>
          <button
            className={lang === 'zh' ? styles.langActive : styles.langToggle}
            onClick={() => setLang('zh')}
          ...
          <button
            className={lang === 'en' ? styles.langActive : styles.langToggle}
            onClick={() => setLang('en')}
